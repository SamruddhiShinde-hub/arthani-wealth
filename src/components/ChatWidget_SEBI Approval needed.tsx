...
import { MessageCircle, X, RotateCcw, Mic, MicOff, Volume2, VolumeX, Shield } from "lucide-react";
...
export default function ChatWidget() {
  ...
  const [reviewedMessages, setReviewedMessages] = useState<Array<{ id: string, message: string, status: string }>>([]);
  ...
  const sendMessageWithInput = async (inputText: string) => {
    ...
    try {
      ...
      const botResponse = data.requiresHumanReview
        ? "🔄 This response requires advisor review for accuracy. Please wait for human verification."
        : data.reply || data.output || data.response || data.message || data.text || "I couldn't process that. Please try again.";

      const botMsg = {
        type: "bot",
        text: botResponse,
        time: new Date().toLocaleTimeString(),
        status: data.requiresHumanReview ? 'pending' : 'approved',
                ...
                >
                  ...
                  {msg.type === "bot" && (
                    <div className="flex items-center gap-2 text-xs text-gray-500 mt-1">
                      <Shield size={12} />
                      <span>SEBI Registered Advisor Verified</span>
                      <span className="ml-auto text-xs">{msg.status === 'approved' ? 'Verified' : 'Pending Review'}</span>
                    </div>
                  )}
                </div>
              </div>
